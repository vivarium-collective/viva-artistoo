...
- **Generate from natural language** — describe what you want; the dashboard writes a request file and prompts you to run `/pbg-viz <name>`, which scaffolds a new Visualization function into `pbg_artistoo/visualizations/` and commits it on a stage branch.
...
`pbg_artistoo` package and the HEAD must be pushed.**
...
2. Launch from the dashboard CLI:
   ...
   - Export the composite to a portable `.pbg` file (full import-path
     addresses: `local:!pbg_artistoo.<Module>.<ClassName>`).
   - Submit to sms-api with `extra_pip_deps=["git+<origin>@<sha>"]` so
     the Singularity container installs `pbg_artistoo` from the
...
**One-time setup:** after the first publish run creates the `gh-pages` branch, go to **Settings → Pages → Source = "Deploy from a branch" → `gh-pages` / `(root)`**. The dashboard is then served at `https://<user>.github.io/artistoo/dashboard/` and reports at `…/artistoo/investigations/<slug>.html`.
